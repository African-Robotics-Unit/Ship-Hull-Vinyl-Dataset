...
std::queue<sensor_msgs::CompressedImage> img0_queue, img1_queue;
int synchs_cnt, i, j = 0; 
int cut = 2000;


//-------------------------FUNCTIONS-------------------------------------------------------
void synchFilterCallback(const sensor_msgs::CompressedImage::ConstPtr& img0_msg, const sensor_msgs::CompressedImage::ConstPtr& img1_msg)
...
{
  sensor_msgs::CompressedImage img0_msg, img1_msg;
  ...
  if(!img0_queue.empty() && !img1_queue.empty())
  {
    ...
    cv_bridge::CvImagePtr cv_ptr_0, cv_ptr_1;
    cv_ptr_0 = cv_bridge::toCvCopy(img0_msg, sensor_msgs::image_encodings::BGR8);
    cv_ptr_1 = cv_bridge::toCvCopy(img1_msg, sensor_msgs::image_encodings::BGR8);
    sensor_msgs::Image img0_msg_decompress, img1_msg_decompress;
    cv_ptr_0->toImageMsg(img0_msg_decompress);
    cv_ptr_1->toImageMsg(img1_msg_decompress);

    // Write a synched pair of messages to a rosbag
    synched_bag.write(cam0_topic, img0_msg.header.stamp, img0_msg_decompress); 
    synched_bag.write(cam1_topic, img1_msg.header.stamp, img1_msg_decompress);
  }
}
...
// Load rosbag, iterate through the messages on each topic and call the synchronizer callback
{
  ...
  message_filters::Subscriber<sensor_msgs::CompressedImage> img0_sub(nh, cam0_topic, 10); 
  message_filters::Subscriber<sensor_msgs::CompressedImage> img1_sub(nh, cam1_topic, 10);
  
  // Use an approximate time synchronizer to synchronize image messages
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::CompressedImage, sensor_msgs::CompressedImage> approxTimePolicy;
  ...
  BOOST_FOREACH(rosbag::MessageInstance const msg, rosbagView)
  {
	if(i<cut){
		...
		{
		  sensor_msgs::CompressedImage::ConstPtr img0 = msg.instantiate<sensor_msgs::CompressedImage>();
		  if (img0 != NULL)
	...
	if(j<cut){
		...
		{
		  sensor_msgs::CompressedImage::ConstPtr img1 = msg.instantiate<sensor_msgs::CompressedImage>();
		  if (img1 != NULL)
			img1_sub.signalMessage(img1); 
			j += 1;
		}
	}
	if(i>=cut && j>=cut){break;}
    writeToBag(synched_bag); // write to the rosbag (disk) and empty the image queues as callbacks are made to save RAM space
	cout << "\rWritten " << i << " of " << cut << " messages." << endl;
  }
  unsynched_bag.close();
  synched_bag.close();
